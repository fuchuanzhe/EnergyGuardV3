...
public class MainViewModelTests
{
    ...
    public async Task TestGetCost(int value)
    {
        ...
        var data = new EnergyUsageData(1.0, 0.5, "EON ENERGY", diaries);
        Assert.IsNotNull(data);
        var databaseService = new Mock<IDatabaseService>();
        databaseService.Setup(d => d.LoadUsageData()).ReturnsAsync(data);
        var energyUsageModel = new EnergyUsageModel(new CarbonIntensityInfo(), new EnergyRateInfo(), databaseService.Object, new MediatorService());
    ...
    public async Task TestSelectMode()
    {
        ...
        var model = new MainViewModel(new PowerInfo(), new CpuInfo(), new GpuInfo(), settingsService,
            _appNotificationService.Object, new EnergyUsageModel(new CarbonIntensityInfo(), new EnergyRateInfo(), new DatabaseService("testDB.db"), new MediatorService()));
