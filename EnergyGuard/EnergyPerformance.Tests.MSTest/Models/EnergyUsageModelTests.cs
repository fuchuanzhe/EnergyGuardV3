...
public class EnergyUsageModelTests
{
    ...
    private static EnergyUsageData generateRandomUsageData(float power, int hourOffset)
    {
        ...
        var energyUsageData = new EnergyUsageData(1, 1, "EDF ENERGY", diaries);
    ...
    public async Task TestModelIsInitializedCase1()
    {
        ...
        var model = new EnergyUsageModel(new CarbonIntensityInfo(), new EnergyRateInfo(), databaseService.Object, new MediatorService());
        await model.InitializeAsync();
        Assert.AreEqual(model.AccumulatedEnergy, 0);
        Assert.AreEqual(model.AccumulatedEnergyHourly, 0);
    }
    ...
    public async Task TestModelIsInitializedCase2()
    {
        ...
        Assert.AreEqual((float)model.AccumulatedEnergy / 3600000, (float)power, tolerance);
        Assert.AreEqual((float)model.AccumulatedEnergyHourly / 3600000, (float)power,tolerance);
    }
    ...
    public async Task TestModelIsInitializedCase3()
    {
        ...
        Assert.AreEqual((float)model.AccumulatedEnergy / 3600000, (float)power, tolerance);
        Assert.AreEqual((float)model.AccumulatedEnergyHourly / 3600000, 0);
    }
    ...
    public async Task TestModelUpdate()
    {
        ...
        var model = new EnergyUsageModel(carbonIntensityInfo, energyRateInfo, databaseService, new MediatorService());
        var preData = await databaseService.LoadUsageData();
        var power = RandomDouble(0.5, 2.0);
        model.AccumulatedEnergy = power;
        await model.Save();
