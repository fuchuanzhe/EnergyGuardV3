...
public partial class SettingsViewModel : ObservableRecipient
{
    ...
    public double CostPerKwh
    {
        ...
        set
        {
            if (double.IsNaN(value) || value <= 0.01)
            {
                // update numberbox again incase the 'X' button was pressed to make the value reappear
    ...
    public SettingsViewModel(IThemeSelectorService themeSelectorService, CpuInfo cpuInfo, ILocalSettingsService settingsService, EnergyUsageModel model, IMediatorService mediatorService, CompanyService companyService)
    {
        _model = model;
        ...
        _mediatorService.SelectedCompanyChanged += OnSelectedCompanyChanged;
        CompanyList = _companyService.CompanyList;
    }

    /// <summary>
    /// Loads the company information from popup to settings page via mediator service, activates with an event.
    /// </summary>
    private void OnSelectedCompanyChanged(object sender, string selectedCompany)
    {
