...
public partial class HistoryViewModel : ObservableObject
{
    ...
    // Add property for storing selected chart type
    private string _selectedChartType;
    private ColumnSeries<DateTimePoint> _lineSeries;
    public string SelectedChartType
    {
        ...
        set
        {
            ...
            Debug.WriteLine(_selectedChartType);
            UpdateChartType(value);
        }
    }

    // Add property for chart type options
    public ObservableCollection<string> ChartTypes { get; } = new ObservableCollection<string> { "Bar", "Line" };
    ...
    private void UpdateChartType(string chartType)
    {
        ...
        {
            case "Bar":
                HistorySeries = new ISeries[] { _historySeries };
                break;
            case "Line":
                HistorySeries = new ISeries[] { _lineSeries };
                break;
            default:
                break;
        }
    ...
    public HistoryViewModel()
    {
        ...
        _historySeries = new ColumnSeries<DateTimePoint>
        {
            ...
            Name = "Watt",
            Values = values,
            Fill = new SolidColorPaint(new SKColor(1, 150, 59))
        };
        _lineSeries = new ColumnSeries<DateTimePoint>
        {
            // You might need to adjust these properties according to your requirements
            YToolTipLabelFormatter = (chartPoint) =>
                $"{chartPoint.PrimaryValue.ToString("F4")} watt",
            Name = "Line",
            Values = values, // Make sure you provide appropriate values here
            Fill = new SolidColorPaint(new SKColor(1, 150, 59))
        };
