...
public partial class CarbonEmissionViewModel : ObservableRecipient
{
    ...
    public double CarbonIntensity => _model.CarbonIntensity;

    public double DailyCarbonEmission => _model.GetDailyCarbonEmission();

    public float EmissionsThisWeek => _model.GetEmissionForCurrentWeek();
    ...
    /// </summary>
    public double TimeToAbsorbEmission => EmissionsThisWeek / ((25000.0 / 365.0) / 24.0);
    ...
    , ILocalSettingsService settingsService, IAppNotificationService notificationService, EnergyUsageModel model, CarbonIntensityInfo carbonIntensity)
    {
        ...
        _carbonIntensityInfo.CarbonIntensityChanged += CarbonIntensityUsage_PropertyChanged;
    ...
    private void CarbonIntensityUsage_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(_carbonIntensityInfo.CarbonIntensity))
        {
            OnPropertyChanged(nameof(CarbonIntensity));
        }
        return;
    }
