...
public class DatabaseService : IDatabaseService
{
    ...
    public async Task InitializeDB()
    {
        if (!MethodWrapper.FileExists(_datasource))
        {
            try
            {
                ...
                Debug.WriteLine("Creating the database");
                ...
                string CreateDiaryTable = "CREATE TABLE energy_diary_log (diary_log_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, " +
                    "date TEXT NOT NULL, exact_date_time TEXT NOT NULL, daily_log_id TEXT, cost_per_kwh NUMERIC, weekly_budget NUMERIC, company TEXT," +
    ...
    public async Task InsertHourlyLog(int hour, EnergyUsageLog data)
    {
        try
        {
            ...
            if (hour > 0 && hour < 7) {
                string companySelected = RetrieveLatestCostPerKwhBudgetCompany().Item3;
                Company companyData = CompanyInfo.GetCompanyData().EnergyDistributors.Find(c => c.Name == companySelected);

                float modifiedCost = data.Cost * companyData.Perks.NightRate;

                SQLiteCommand updateCmd = new SQLiteCommand($"UPDATE energy_usage_log SET cost = {modifiedCost} WHERE log_id = @id", conn);
    ...
    private async Task UpdateBudgetCostPerKwhCompany(string date, double costPerKwh, double weeklyBudget, string company)
    {
        SQLiteConnection conn = CreateConnection();
        string query = $"UPDATE energy_diary_log SET cost_per_kwh = {costPerKwh}, " +
            $"weekly_budget = {weeklyBudget}, company = \"{company}\" WHERE date = \"{date}\"";
    ...
    public async Task SaveEnergyData(EnergyUsageData data)
    {
        try
        {
            ...
            foreach (EnergyUsageDiary diary in data.Diaries)
            {
                if (!(currentUsageData.Diaries.Contains(diary)))
                {
                    ...
                    await UpdateBudgetCostPerKwhCompany(currentDate, data.CostPerKwh, data.WeeklyBudget, data.Company);
                }
            }
            _energyUsage = data;
        }
        ...
        {
            Debug.WriteLine(ex.Message + " in SaveEnergyData");
        }
    }
    ...
    public (double, double, string) RetrieveLatestCostPerKwhBudgetCompany()
    {
        try
        {
            ...
            string query = "SELECT cost_per_kwh, weekly_budget, company FROM energy_diary_log ORDER BY diary_log_id DESC LIMIT 1";
            ...
            reader.Close();
            connection.Close();
            return (costPerKwh, weeklyBudget, company);
        }
        catch(Exception ex)
        {
            Debug.WriteLine(ex.Message + " in RetrieveLatestCostBudget");
    ...
    private async Task<EnergyUsageData> ReadUsageDataFromDatabase()
    {
        try
        {
            ...
            double weeklyBudget;
            string company;
            (costPerKwh, weeklyBudget, company) = await Task.Run(() => RetrieveLatestCostPerKwhBudgetCompany());
            EnergyUsageData energyUsage = new EnergyUsageData(costPerKwh, weeklyBudget, company, diaries);
            return energyUsage;
        }
        catch(Exception ex)
        {
            Debug.WriteLine(ex.Message + " in readusagedata");
            return null;
        }
    }
