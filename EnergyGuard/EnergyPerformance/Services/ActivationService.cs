...
public class ActivationService : IActivationService
{
    ...
    public async Task ActivateAsync(object activationArgs)
    {
        ...
        // Set the MainWindow Content, the part where the application window is picked and initialized
        if (App.MainWindow.Content == null)
        {
            if (_energyUsageModel.IsLiveCost)
            {
                _startpage = App.GetService<ShellPage>();
            } 
            else
            {
                _startpage = App.GetService<PopupPage>();

            }
            App.MainWindow.Content = _startpage ?? new Frame();
        }
